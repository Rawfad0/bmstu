class DES:
    ...
    def generate_subkeys(self):
        ...
        #print(f"Ключ (56 бит): {len(key_bits)} бит")

        # Применяем PC1 к 56-битному ключу
        #key_pc1 = self.permute(key_bits, self.PC1)
        #print(f"После PC1: {len(key_pc1)} бит")

        # Разделение на левую и правую части по 28 бит
        left = key_bits[:28]
        right = key_bits[28:]

        print(f"Левая часть: {len(left)} бит, Правая часть: {len(right)} бит")
...
def text_to_binary_string(text):
    ...
    binary_chars = []
    for char in text:
        binary_chars.append(bin(ord(char))[2:].zfill(8))
    return ''.join(binary_chars)


def compare_binary_representations(original, encrypted, decrypted):
    """Сравнение бинарных представлений"""
    print("\n" + "=" * 80)
    print("СРАВНЕНИЕ БИНАРНЫХ ПРЕДСТАВЛЕНИЙ")
    print("=" * 80)

    # Преобразуем в бинарные строки
    orig_bin = text_to_binary_string(original)
    enc_bin = text_to_binary_string(encrypted)
    dec_bin = text_to_binary_string(decrypted)

    print(f"1. ИСХОДНЫЙ ТЕКСТ:")
    print(f"   Длина: {len(orig_bin)} бит")
    print(f"   {format_binary_display([int(bit) for bit in orig_bin])}")

    print(f"\n2. ЗАШИФРОВАННЫЙ ТЕКСТ:")
    print(f"   Длина: {len(enc_bin)} бит")
    print(f"   {format_binary_display([int(bit) for bit in enc_bin])}")

    print(f"\n3. РАСШИФРОВАННЫЙ ТЕКСТ:")
    print(f"   Длина: {len(dec_bin)} бит")
    print(f"   {format_binary_display([int(bit) for bit in dec_bin])}")
...
def show_key_info(key):
    ...
    for char in key:
        binval = bin(ord(char))[2:].zfill(8)
        key_bits.extend([int(x) for x in list(binval)])

    print(f"Ключ: '{key}'")
    print(f"Длина ключа: {len(key)} символов = {len(key_bits)} бит")
    print(f"Бинарное представление ключа:")
    print(f"   {format_binary_display(key_bits)}")


def main():
    print("=== АЛГОРИТМ DES С 56-БИТНЫМ КЛЮЧОМ ===")
    print("Реализация по схеме Фейстеля с 16 раундами")
    print("Последний раунд без замены (первый способ)")

    key = "\x00\x00\x00\x00\x00\x00\x00"
    plaintext = "arbuziki bananchiki nektarinki"  # 5 символов для теста
    plaintext = "hello"

    show_key_info(key)
    print(f"Исходный текст: '{plaintext}'")
    print(f"Длина текста: {len(plaintext)} символов = {len(plaintext) * 8} бит")
    ...
    try:
        ...
        # Сравнение бинарных представлений
        compare_binary_representations(plaintext, ciphertext, decrypted)
        ...
        print(f"Совпадение текстов: {plaintext == decrypted}")
