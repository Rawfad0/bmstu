...
i=1
num_TT $i
while [ -f ./func_tests/data/pos_"$TT"_in.txt ] && [ -f ./func_tests/data/pos_"$TT"_out.txt ]; do
    FILE_IN=func_tests/data/pos_"$TT"_in.txt
    FILE_OUT=func_tests/data/pos_"$TT"_out.txt

    if ./func_tests/scripts/pos_case.sh "$FILE_IN" "$FILE_OUT"; then
...
while [ -f ./func_tests/data/neg_"$TT"_in.txt ]; do
    ...
    if ./func_tests/scripts/neg_case.sh "$FILE_IN" "$FILE_OUT"; then
