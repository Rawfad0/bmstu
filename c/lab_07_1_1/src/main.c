 ...
 *
 * Кроме того, необходимо сравнить время работы реализованного алгоритма сортировки и qsort. 
 * Постарайтесь найти примеры, когда ваша реализация лучше. Результаты представьте в виде графиков 
 * (рисунки в формате PNG или любом аналогичном векторном, оси подписаны). 
 * Графики разместить в Wiki на отдельной странице.
 * Графики можно строить любыми средствами (например, в Excel, gnuplot).
 * 
 */
...
int main(int argc, char **argv)
{
    if (argc != ARGC_NO_FILTER && argc != ARGC_FILTER)
        return RETURN_CODE_INCORRECT_ARGUMENT_COUNT;    // некорректное количество аргументов
    ...
    if (!(rc = count_int_arr_len(FILE_IN, &n)))
    {
        ...
        if (!pb)
            rc = RETURN_CODE_ALLOCATION_FAILED;
        else 
        {
            int *pe = pb + n;
...
int controller(int argc, char **argv, int *pb, int *pe)
{
    ...
    else if (argc == ARGC_NO_FILTER)
    {
        ...
        rc = RETURN_CODE_INCORRECT_ARGUMENT;
    return rc;
}
