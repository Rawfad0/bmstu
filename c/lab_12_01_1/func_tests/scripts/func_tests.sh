...
EXE=$1
echo "Функциональное тестирование исполняемого файла $EXE"
...
while [ -f ./func_tests/data/pos_"$TT"_in.txt ] \
    ...
    if ./func_tests/scripts/pos_case.sh "$EXE" "$FILE_IN" "$FILE_OUT" "$FILE_ARGS"; then
...
while [ -f ./func_tests/data/neg_"$TT"_in.txt ] \
    ...
    if ./func_tests/scripts/neg_case.sh "$EXE" "$FILE_IN" "$FILE_OUT" "$FILE_ARGS"; then
