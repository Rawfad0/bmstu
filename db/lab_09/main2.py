...
def add_random_purchase():
    global flag
    cursor.execute("select COALESCE(MAX(user_id), 0) + 1 FROM users_redis")
    user_id = cursor.fetchone()[0]
    cursor.execute("insert into users_redis values(%s, 'Lastname', 'Firstname', 'phonenumber', 'email', '2024-12-12', '2024-12-12');", (user_id, ))
    conn.commit()
...
def delete_random_purchase():
    while True:
        ...
        cursor.execute("select COUNT(*) from users_redis where user_id = %s", (random_id,))
        exists = cursor.fetchone()[0] > 0
        if exists:
            cursor.execute("delete from users_redis where user_id = %s", (random_id,))
            conn.commit()
            global flag
            flag = False
            break
...
def update_random_purchase():
    while True:
        ...
        exists = cursor.fetchone()[0] > 0
        if exists:
            cursor.execute("update users_redis set email = '@' WHERE user_id=%s", (random_id, ))
            conn.commit()
            global flag
            flag = False
            break
...
def graph_update():
    ...
    cache_thread = Thread(target=cache_query)
    db_thread = Thread(target=db_query)
    update_thread = Thread(target=data_update)
