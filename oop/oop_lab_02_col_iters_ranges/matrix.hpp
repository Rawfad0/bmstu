...
{
    _allocate(rows * columns);
    // std::ranges::fill(this->begin(), this->end(), filler);
    for (int i : std::ranges::iota_view(0, int(rows*columns)))
        _data[i] = filler;
...
Matrix<value_type>::Matrix(const Matrix<value_type> &matrix): BaseMatrix(matrix._rows, matrix._cols) 
{
    ...
    for (size_type i : std::ranges::iota_view(0, int(_rows)))
        for (size_type j : std::ranges::iota_view(0, int(_cols)))
...
{
    // std::ranges::copy(matrix.begin(), matrix.end(), std::ostream_iterator<value_type>(std::cout, " "));
    // os << std::endl;
