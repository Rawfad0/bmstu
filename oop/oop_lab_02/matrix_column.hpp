...
template <MatrixType value_type>
Matrix<value_type>::iterator Matrix<value_type>::MatrixColumn::begin() 
{
    return Iterator<value_type>(this->_matrix, this->_index);
}

template <MatrixType value_type>
Matrix<value_type>::iterator Matrix<value_type>::MatrixColumn::end() 
{
    return Iterator<value_type>(this->_matrix, (this->_matrix._cols) * (this->get_size() - 1) + this->_index + 1);
}
