from ant_alg import ant_algorithm, print_route, ANT, FULL, BOTH
from std_alg import bruteforce
from mtx_t import input_matrix, get_file_matrix, print_matrix, get_random_matrix
...
def menu(alg_type):
    ...
    if alg_type == FULL or alg_type == BOTH:
        route1, cost1 = bruteforce(matrix)
...
def parameterization():
    ...
    _, ideal1 = bruteforce(matrix1)
    _, ideal2 = bruteforce(matrix2)
    _, ideal3 = bruteforce(matrix3)
    result1 = open("../data/m1_result.txt", "w")
    result2 = open("../data/m2_result.txt", "w")
    result3 = open("../data/m3_result.txt", "w")
    result_all = open("../data/results.txt", "w")
    sep = " & "
    ender = " \\\\ \\hline"
    ...
    for a in alpha:
        ...
        for p in evaporation:
            ...
            for day in days:
                ...
                str1 = f"{a:4.1f}{sep}{p:4.1f}{sep}{day:4d}{sep}{ideal1:5d}{sep}{first - ideal1:5d}{ender}\n"
                str2 = f"{a:4.1f}{sep}{p:4.1f}{sep}{day:4d}{sep}{ideal2:5d}{sep}{second - ideal2:5d}{ender}\n"
                str3 = f"{a:4.1f}{sep}{p:4.1f}{sep}{day:4d}{sep}{ideal3:5d}{sep}{third - ideal3:5d}{ender}\n"
                str_all = (f"{a:4.1f}{sep}{p:4.1f}{sep}{day:4d}{sep}"
                           f"{first - ideal1:5d}{sep}{second - ideal2:5d}{sep}{third - ideal3:5d}{sep}"
                           f"{first - ideal1:5d}{sep}{second - ideal2:5d}{sep}{third - ideal3:5d}{sep}"
                           f"{first - ideal1:5d}{sep}{second - ideal2:5d}{sep}{third - ideal3:5d}{sep}"
                           f"{ender}\n")
                result1.write(str1)
...
def read_file(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            data.append(line_data)  # Append the line data to the list
    return data
...
def compare_files(file1, file2, file3):
    ...
    for i in range(len(data1)):
        row1 = [float(value.strip(' \\\\ \\hline')) for value in data1[i]]
        row2 = [float(value.strip(' \\\\ \\hline')) for value in data2[i]]
        row3 = [float(value.strip(' \\\\ \\hline')) for value in data3[i]]
    ...
    for i in range(10):
        ...
        print(f" {data1[idx][0]} & {data1[idx][1]} & {data1[idx][2]} "
              f"& {data1[idx][4]} & {data1[idx][4]} & {data1[idx][4]} "
              f"& {data2[idx][4]} & {data2[idx][4]} & {data2[idx][4]} "
              f"& {data3[idx][4]} & {data3[idx][4]} & {data3[idx][4]}".replace("\\\\ \\hline", "") + "\\\\")
