...
void measure_search(const char *key, void *n)
{
    if (key == NULL)
        return;
...
int compare_cmps(char *filename, bin_tree_t tree)
{
    ...
    while (!feof(f) && rc == RC_OK && rc_bt == BIN_TREE_OK)
    {
        if (get_input(&tmp, f) != RC_OK)
            break;
        // Замер поиска в файле
        rc = find_in_file(filename, tmp, &cmp_count_file);
        // Замер поиска в дереве
        rc_bt = bin_tree_find(tree, tmp, &cmp_count_tree);
...
void count_mem(const char *key, void *n)
{
    int *mem_counter = n;
    if (key != NULL)
